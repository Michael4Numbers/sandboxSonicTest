...
PS
{
	...
	float4 MainPs( PixelInput i ) : SV_Target0
	{
		...
		float2 l_0 = float2( 0.5, 0.5 );
		float l_1 = l_0.x * -1;
		float2 l_2 = i.vTextureCoords.xy * float2( 1, 1 );
		float2 l_3 = float2( l_1, l_1 ) + l_2;
		float l_4 = g_flSpeed;
		float l_5 = l_4 * g_flTime;
		float l_6 = cos( l_5 );
		float l_7 = sin( l_5 );
		float l_8 = l_7 * -1;
		float2 l_9 = float2( l_6, l_8 );
		float l_10 = dot( l_3, l_9 );
		float2 l_11 = float2( l_7, l_6 );
		float l_12 = dot( l_3, l_11 );
		float2 l_13 = float2( l_10, l_12 );
		float2 l_14 = l_0 + l_13;
		float4 l_15 = Tex2DS( g_tTexture, g_sSampler0, l_14 );
